---
title: 示例服务器
description: '一系列示例服务器及其实现'
---

本页面展示了多种 Model Context Protocol (MCP) 服务器，展示了该协议的功能和灵活性。这些服务器使得大型语言模型（LLM）能够安全地访问工具和数据源。

## 参考实现

以下官方参考服务器展示了 MCP 的核心功能和 SDK 使用方式：
...
- **[Google Drive](https://github.com/modelcontextprotocol/servers/tree/main/src/gdrive)** - 支持 Google Drive 的文件访问和搜索功能
...
- **[Sentry](https://github.com/modelcontextprotocol/servers/tree/main/src/sentry)** - 从 Sentry.io 获取并分析问题

### Web 和浏览器自动化
- **[Brave Search](https://github.com/modelcontextprotocol/servers/tree/main/src/brave-search)** - 使用 Brave 的搜索 API 进行 Web 和本地搜索
- **[Fetch](https://github.com/modelcontextprotocol/servers/tree/main/src/fetch)** - 针对 LLM 优化的 Web 内容抓取和转换
- **[Puppeteer](https://github.com/modelcontextprotocol/servers/tree/main/src/puppeteer)** - 浏览器自动化和 Web 抓取能力

### 生产力和通信
- **[Slack](https://github.com/modelcontextprotocol/servers/tree/main/src/slack)** - 渠道管理和消息传递功能
- **[Google Maps](https://github.com/modelcontextprotocol/servers/tree/main/src/google-maps)** - 地理位置服务、路线规划和地点详情
...
- **[AWS KB Retrieval](https://github.com/modelcontextprotocol/servers/tree/main/src/aws-kb-retrieval-server)** - 使用 Bedrock Agent Runtime 从 AWS 知识库中检索信息

## 官方集成

以下 MCP 服务器由公司维护，适用于其平台：
...
- **[Obsidian Markdown Notes](https://github.com/calclavia/mcp-obsidian)** - 阅读和搜索 Obsidian 保管库中的 Markdown 笔记
- **[Qdrant](https://github.com/qdrant/mcp-server-qdrant/)** - 通过 Qdrant 向量搜索引擎实现语义记忆
...
- **[Tinybird](https://github.com/tinybirdco/mcp-tinybird)** - 与 Tinybird 无服务器 ClickHouse 平台进行交互
- **[Weaviate](https://github.com/weaviate/mcp-server-weaviate)** - 通过 Weaviate 集合实现 Agentic RAG

## 社区亮点

越来越多的社区开发服务器扩展了 MCP 的能力：
...
> **注意：** 社区服务器未经测试，请自行承担风险。它们并未与 Anthropic 关联或获得其支持。

欲查看完整的社区服务器列表，请访问 [MCP 服务器仓库](https://github.com/modelcontextprotocol/servers)。
...
基于 Python 的服务器可以通过 `uvx`（推荐）或 `pip` 使用：
...
要将 MCP 服务器与 Claude 配合使用，可以将其添加到配置中：
...
- [MCP 服务器仓库](https://github.com/modelcontextprotocol/servers) - 完整的参考实现和社区服务器集合
- [Awesome MCP 服务器](https://github.com/punkpeye/awesome-mcp-servers) - 精心挑选的 MCP 服务器列表
